...
export default function Aside({ data }: AsideProps) {
  return (
    <aside className="flex flex-col justify-between min-w-35 h-[calc(100dvh-16px)] rounded-md p-1 border-[0.5px] border-gray/25">
        ...
        {data.map((link) => {
          ...
          return (
            <Link
              key={link.label}
              href={link.href}
              className="group/nav-link flex items-center gap-2 h-[30px] rounded-md hover:bg-card px-1"
            >
      ...
      <button className="group/settings flex items-center gap-2 h-[30px] rounded-md hover:bg-card px-1">
